...
const server = app.listen(3030, () => {
  logger.info(`Listening: http://localhost:${3030}`);
});
...
function onShutdown() {
  ...
  evalJobCreator?.close().then(() => logger.info('Eval Job Creator has been closed.'));
  evalJobExecutor?.close().then(() => logger.info('Eval Job Executor has been closed.'));
}
